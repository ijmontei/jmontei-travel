...
function ItineraryPanel({ posts }: { posts: Post[] }) {
  ...
  const [q, setQ] = useState<string>(""); // optional search
  ...
  // Optional UX improvement: city options narrowed by selected country (except "All")
  ...
  // Latest (for "Current location") – global, not filtered (feels better for family)
  const latest = useMemo(() => {
    ...
    const cty = (newest as any)?.city as string | undefined;
    const ctry = newest?.country as string | undefined;
    ...
    <section className="mt-6">
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm text-zinc-500">
          Filter by country, city, date range, or activity to find exactly what you want.
        </p>
      </div>
      ...
      <div className="mt-5 rounded-2xl border bg-white/70 backdrop-blur p-4 shadow-sm">
        <div className="grid gap-3 md:grid-cols-5">
          ...
          <button
            ...
            className="rounded-xl border bg-[#414141] px-3 py-2 text-sm font-semibold text-[#f5de88]"
          >
            Clear
          </button>
        </div>
          ...
          <div className="mt-6 relative">
            ...
            <div className="space-y-5">
              {byDay.map(({ day, items }, idx) => {
                ...
                // NOTE: byDay is DESC (newest -> oldest). "Previous" item in the list is idx-1 (newer day).
