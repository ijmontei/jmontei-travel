...
export function PostCard(props: PostCardProps) {
  const { title, slug, excerpt, coverImage, publishedAt, tripTitle } = props;

  const imageUrl = coverImage ? urlForImage(coverImage).width(1200).height(700).fit("crop").url() : null;
      ...
      <div className="rounded-2xl border overflow-hidden bg-zinc-50">
        ...
        <div className="p-6">
          <div className="flex flex-wrap gap-x-3 gap-y-1 items-center text-xs text-zinc-500">
            {tripTitle && <span className="rounded-full bg-white px-2 py-1 border">{tripTitle}</span>}
